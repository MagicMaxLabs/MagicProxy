...
У меня в браузере несколько профилей. Рабочий, личный и пара под конкретные задачи.
А любой VPN-клиент умеет только «включено» и «выключено» — на всю систему сразу.
Каждое переключение между профилями превращалось в дёрганье тумблера, и рано или
поздно я забывал, в каком он состоянии.
...
Не заворачивать в туннель весь компьютер, если нужен только браузер. И просто удобно
пользоваться своим сервером, не открывая отдельную программу.

**Чем отличается от v2rayN, Hiddify и подобных.** Те проксируют всю систему.
MagicProxy занимает один профиль браузера и спокойно уживается с уже установленным
клиентом — у меня они работают одновременно.
