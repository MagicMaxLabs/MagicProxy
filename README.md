...
распакованное** и укажи папку `extension`. Идентификатор получится тот же, что у
магазинной сборки, — публичный ключ лежит в манифесте.
...
При включённом прокси расширение закрывает и обходной путь WebRTC: политика
`disable_non_proxied_udp` не даёт странице узнать реальный адрес через STUN, а при
выключении снимается. Интерфейс — на русском и английском, язык берётся из браузера.

Отдельно стоит прочитать, чего MagicProxy **не** делает: это не анонимность, креды
серверов хранятся в открытом виде, у звонков в проксированном профиле нет прямого UDP.
Всё это разобрано в [SECURITY.md](SECURITY.md), там же сказано, что с этим делать.
Формальная политика приватности — [PRIVACY.md](PRIVACY.md).
